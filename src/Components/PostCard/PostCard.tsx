...
const PostCard = ({ id, postResponse }: Props) => {
  ...
  return (
    <div id={id} key={id} className="max-w-sm mb-3 p-6 bg-white border border-gray-200 rounded-lg shadow">
      ...
      <Link to={`/post/${postResponse.id}`} className="inline-flex items-center px-3 py-2 text-sm font-medium text-center text-white bg-green-700 rounded-lg hover:bg-green-800 focus:ring-4 focus:outline-none focus:ring-green-300">
        Read
      </Link>
    </div>
  );
